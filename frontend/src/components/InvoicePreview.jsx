...
export default function InvoicePreview({ formData, items, totals, onSaved }) {
  ...
  const [selectedCopies, setSelectedCopies] = useState({ Original: true, Duplicate: false, Transport: false });
  ...
  const handleCheckboxChange = (type) => {
    ...
    if (type === 'Original') setActiveCopyText('ORIGINAL FOR RECIPIENT');
    else if (type === 'Duplicate') setActiveCopyText('DUPLICATE FOR RECIPIENT');
    else if (type === 'Transport') setActiveCopyText('TRANSPORT FOR RECIPIENT');
  };
    ...
    <div className="rhs-preview-container">
      ...
      <div className="a4-paper" ref={invoiceRef} style={dynamicTableStyles}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 'var(--g-mar)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', backgroundColor: 'transparent', marginTop: '4px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '2px solid black', borderBottom: '1px solid black', padding: 'calc(var(--r-pad) * 1.5) var(--r-pad)', fontSize: 'var(--f-row)', fontWeight: '900', color: '#000' }}>
          ...
          <div style={{ flex: 1, textAlign: 'center', fontSize: 'calc(var(--f-hdr) * 1.4)', letterSpacing: '2px' }}>TAX INVOICE</div>
          <div style={{ flex: 1, textAlign: 'right', fontSize: 'var(--f-hdr)' }}>{activeCopyText}</div>
        </div>

        {/* 2-Column Info Table Wrapper */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px solid black', borderLeft: '1px solid black', borderRight: '1px solid black', backgroundColor: 'transparent', margin: 0 }}>
          ...
          <div style={{ borderRight: '1px solid black', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: 'var(--r-pad)', fontSize: 'var(--f-row)', display: 'grid', gridTemplateColumns: '85px 1fr', gap: '2px', flex: 1 }}>
              <strong style={{color: '#000'}}>M/S</strong> <div style={{color: '#000'}}>{formData.customer_name || 'Customer Name'}</div>
              <strong style={{color: '#000'}}>Address</strong> <div style={{color: '#000', whiteSpace: 'pre-wrap'}}>{formData.customer_address || 'Customer Address here...'}</div>
